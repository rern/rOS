...
9 "$upmpdcli" on )
	...
	[[ $select == *' 2 '* ]] && features+='chromium matchbox-window-manager plymouth-lite-rbp upower xf86-input-evdev xf86-video-fbdev xf86-video-vesa xinput_calibrator xorg-server xorg-xinit ' && list+="$browser"$'\n'
...
curl -skL https://github.com/archlinuxarm/PKGBUILDs/raw/master/core/pacman-mirrorlist/mirrorlist -o $ROOT/etc/pacman.d/mirrorlist
readarray -t lines <<< $( grep . $ROOT/etc/pacman.d/mirrorlist \
							| sed -n '/### A/,$ p' \
